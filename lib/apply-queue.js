...
function createApplyQueue(azure) {
  ...
  async function recoverStuckJobs(clientId) {
    let pool = null;
    try {
      pool = createPool();
    } catch {}
    if (pool && typeof pool.query === 'function') {
      try {
        await pool.query(
          `UPDATE dice_apply_queue
           SET status = 'preflight_failed',
               last_error = 'Job timed out or worker crashed during preflight'
           WHERE client_id = $1
             AND status = 'preflight_running'
        ...
        await pool.query(
          `UPDATE dice_apply_queue
           SET status = 'apply_failed',
               last_error = 'Job timed out or worker crashed during apply'
           WHERE client_id = $1
             AND status = 'running'
             AND locked_at < now() - interval '15 minutes'`,
          [clientId]
        ...
        console.error('recoverStuckJobs query failed:', err.message);
      }
    }
  }

      ...
      isManager
        ? db.query(`
            select
              ...
              (select count(distinct al.id)::int from dice_workflow_audit_logs al join dice_sessions ds on ds.telegram_chat_id = al.telegram_chat_id join clients_additional_info c on c.id = ds.client_id join dice_ca_accounts ca on (ca.id::text = c.career_associate_id::text or lower(ca.email) = lower(c.career_associate_id::text)) where al.event = 'job_missed' and ${dateFilterStatsAudit} and ca.manager_id = $1) as jobs_missed
          `, [managerId])
        : db.query(`
            select
              ...
              (select count(*)::int from dice_workflow_audit_logs al where al.event in ('job_yes', 'job_no', 'job_missed') and ${dateFilterStatsAudit}) as jobs_sent,
              ...
              (select count(*)::int from dice_workflow_audit_logs al where al.event = 'job_missed' and ${dateFilterStatsAudit}) as jobs_missed
          `),
      isManager
        ? db.query(`
...
async function readAllClients(requestUrl, response, db, operator) {
  try {
    ...
    const query = isManager
      ? `select distinct on (c.id) c.id as client_id, c.full_name, c.company_email, ds.telegram_chat_id,
                coalesce(nullif(ca.name, ''), ca.email, c.career_associate_id::text, 'Unassigned') as ca_name, ca.id as ca_id
         from clients_additional_info c
         left join dice_sessions ds on ds.client_id = c.id
         left join dice_ca_accounts ca on (ca.id::text = c.career_associate_id::text or lower(ca.email) = lower(c.career_associate_id::text))
         where ca.manager_id = $1
         order by c.id, c.full_name asc nulls last`
      : `select distinct on (c.id) c.id as client_id, c.full_name, c.company_email, ds.telegram_chat_id,
                coalesce(nullif(ca.name, ''), ca.email, c.career_associate_id::text, 'Unassigned') as ca_name, ca.id as ca_id
         from clients_additional_info c
         left join dice_sessions ds on ds.client_id = c.id
    ...
    const [statsRes, jobsRes] = await Promise.all([
      db.query(`
        select
          ...
          (select count(distinct al.id)::int from dice_workflow_audit_logs al join dice_sessions ds on ds.telegram_chat_id = al.telegram_chat_id where ds.client_id = $1 and al.event = 'job_missed' and ${dateFilterAudit}) as skipped_count
      `, [clientId]),
      db.query(`

    ...
    <div className="flex flex-col h-screen bg-black text-slate-200 font-sans overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 bg-black p-6 overflow-y-auto custom-scrollbar">
          ...
          {activeTab === 'workers' && (
            <div>
              <h3 className="text-white text-lg font-medium mb-6">Apply Workers & Queue Status {selectedCandidate !== 'ALL' ? '(Filtered)' : ''}</h3>
                ...
                <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl p-6">
                  ...
                  <div className="space-y-3">
                    {filteredQueue.length === 0 ? (
                      ...
                      filteredQueue.map(q => (
                        <div key={q.id} className="p-4 bg-black rounded-xl border border-white/5">
                          <div className="flex justify-between items-start mb-2">
                            <div className="text-sm text-white font-medium">{q.full_name || q.client_id}</div>
                            <div className="text-xs text-zinc-500">Attempt {q.attempts}/{q.max_attempts}</div>
                          </div>
                          <div className="text-xs text-zinc-400 truncate mb-3" title={q.url}>{q.url}</div>
                          <div className={`text-xs px-2 py-1 inline-block rounded font-bold uppercase tracking-wider ${q.status === 'failed' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'bg-blue-500/10 text-blue-400 border border-blue-500/20'}`}>
                            {q.status}
                          </div>
                          {q.last_error && <div className="mt-2 text-xs text-rose-300 italic">{q.last_error}</div>}
                        </div>
                      ))
                    )}
                  </div>

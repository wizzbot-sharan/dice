    ...
    <div className="flex h-screen bg-[#000000] text-slate-200 font-sans overflow-hidden">
      ...
      >
        ...
        <div className="pb-6 pt-4 border-t border-white/5 space-y-2 min-w-[224px]">
          <Link to="/dev" target="_blank" className="w-full flex items-center space-x-3 px-3 py-2 text-slate-300 hover:bg-white/5 hover:text-white rounded-lg text-sm font-medium transition-colors">
          ...
          <div className="relative mt-2 pt-2 border-t border-white/5">
            ...
            >
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-xs text-slate-500 truncate capitalize">{operator?.role || 'Manager'}</p>
              </div>
            </button>
          ...
          <div className="flex items-center justify-center shrink-0">
            <div className="flex items-center bg-[#111111] px-5 py-2 rounded-full border border-white/10 shadow-sm hover:border-white/20 hover:shadow-md transition-all duration-300 ease-out group">
              ...
              <div className="flex items-center space-x-2">
                ...
                <select value={selectedTimezone} onChange={(e) => setSelectedTimezone(e.target.value)} className="bg-transparent border-none text-slate-300 text-sm focus:ring-0 outline-none cursor-pointer p-0 min-w-[110px]">
                ...
                return (
                  <div key={caName} className="mb-6">
                    ...
                    <button 
                      ...
                      className="w-full text-left bg-[#111111] hover:bg-white/5 border border-white/5 rounded-2xl px-6 py-5 flex items-center justify-between transition-colors shadow-sm group"
                    >
                      <div className="flex items-center space-x-4">
                        <div className={`p-2 rounded-lg transition-colors ${isCaExpanded ? 'bg-indigo-500/20 text-indigo-400' : 'bg-white/5 text-slate-400 group-hover:text-slate-300'}`}>
                      ...
                      <div className="flex items-center space-x-6">
                        <div className="flex items-center space-x-2">
                          <div className="px-3 py-1 bg-white/5 text-slate-400 text-sm font-medium rounded-full">
                    ...
                    {isCaExpanded && (
                      <div className="mt-4 pl-4 space-y-4 border-l-2 border-white/5 ml-6">
                        {candidates.map((user) => {
                          ...
                          return (
                            <div key={user.id} className={`bg-[#111111] border rounded-2xl overflow-hidden shadow-sm transition-all ${isLinked ? 'border-white/10 hover:border-white/20' : 'border-white/5 opacity-60'}`}>
                              <div 
                              ...
                              >
                                <div className="flex items-center space-x-4">
                                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm border ${isLinked ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' : 'bg-white/5 text-slate-300 border-white/10'}`}>
                                  ...
                                  <div>
                                    ...
                                    <p className="text-xs text-slate-500 font-mono mt-0.5">{user.applywizz_id || user.id}</p>
                                  </div>
                                </div>
                                      ...
                                      <div className="animate-in fade-in duration-300">
                                        <div className="grid grid-cols-3 gap-5 mb-10">
                                          ...
                                          <div className="bg-indigo-900/10 p-5 rounded-2xl border border-indigo-500/20">
                                            ...
                                            <p className="text-2xl font-mono text-indigo-400 font-light">--</p>
                                          </div>
                                        </div>
                                        ...
                                        <div>
                                          ...
                                          <div className="grid grid-cols-1 gap-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                                            {user.queue && user.queue.length > 0 ? user.queue.map((q, i) => (
                                              <div key={i} className="bg-[#141414] rounded-xl p-4 border border-white/5 flex justify-between items-center">
                                                <div>
                                                ...
                                                <a href={q.job_url} target="_blank" rel="noreferrer" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium px-4 py-1.5 bg-indigo-500/10 rounded-lg transition-colors">
                                                  View ↗
                                                </a>
                                              </div>
                                            )) : (
                                    ...
                                    {candidateTab === 'jobs' && (
                                      <div className="animate-in fade-in duration-300">
                                        ...
                                        <div>
                                          ...
                                          <div className="bg-[#141414] rounded-xl border border-white/5 divide-y divide-white/5 max-h-[500px] custom-scrollbar overflow-y-auto">
                                            {user.prompt_events && user.prompt_events.length > 0 ? user.prompt_events.map((p, i) => (
                                              <div key={i} className="px-5 py-3 flex items-center justify-between text-sm">
                                                <div className="flex items-center space-x-4">
                                                  <span className="text-slate-500 font-mono text-xs">{new Date(p.sent_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                                  <span className="px-2 py-0.5 bg-white/10 text-white text-[11px] uppercase rounded">{p.decision || 'pending'}</span>
                                                  <span className="text-slate-300 max-w-[200px] truncate">{p.job_title}</span>
                                                </div>
                                                <span className="text-slate-600 text-[11px] truncate">t.me</span>
                                              </div>
                                            )) : (
                                              <p className="text-slate-500 text-sm p-4">No prompts telemetry.</p>
                                            )}
                                          </div>
                                        </div>
                                      </div>
          ...
          {mainTab === 'stats' && data && (
            <div className="max-w-6xl mx-auto animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-5 gap-4 mb-12">
                <div className="bg-[#111111] border border-white/5 p-5 rounded-2xl shadow-sm">
                  <p className="text-[10px] text-slate-500 font-bold mb-2 uppercase tracking-widest">Total Candidates</p>
                  <p className="text-3xl font-medium text-white">{data.global_stats?.total_candidates || 0}</p>
                  <p className="text-xs text-slate-500 mt-2">Ingested & registered</p>
                </div>
                <div className="bg-[#111111] border border-white/5 p-5 rounded-2xl shadow-sm border-b-2 border-b-blue-500/50">
                  <p className="text-[10px] text-slate-500 font-bold mb-2 uppercase tracking-widest">Total Applications</p>
                  <p className="text-3xl font-medium text-blue-400">{data.global_stats?.total_applications || 0}</p>
                  <p className="text-xs text-slate-500 mt-2">Processed jobs</p>
                </div>
                <div className="bg-[#111111] border border-white/5 p-5 rounded-2xl shadow-sm border-b-2 border-b-emerald-500/50">
                  <p className="text-[10px] text-slate-500 font-bold mb-2 uppercase tracking-widest">Accepted (Yes)</p>
                  <p className="text-3xl font-medium text-emerald-400">{data.global_stats?.prompts?.accepted || 0}</p>
                  <p className="text-xs text-slate-500 mt-2">User approved</p>
                </div>
                <div className="bg-[#111111] border border-white/5 p-5 rounded-2xl shadow-sm border-b-2 border-b-red-500/50">
                  <p className="text-[10px] text-slate-500 font-bold mb-2 uppercase tracking-widest">Rejected (No)</p>
                  <p className="text-3xl font-medium text-red-400">{data.global_stats?.prompts?.rejected || 0}</p>
                  <p className="text-xs text-slate-500 mt-2">User declined</p>
                </div>
                <div className="bg-[#111111] border border-white/5 p-5 rounded-2xl shadow-sm border-b-2 border-b-slate-500/50">
                  <p className="text-[10px] text-slate-500 font-bold mb-2 uppercase tracking-widest">Skipped / Expired</p>
                  <p className="text-3xl font-medium text-slate-400">{data.global_stats?.prompts?.skipped || 0}</p>
                  <p className="text-xs text-slate-500 mt-2">Timed out</p>
                </div>
              </div>
              ...
              <div className="bg-[#111111] border border-white/5 rounded-2xl overflow-hidden shadow-sm">
                <div className="px-6 py-5 border-b border-white/5 flex items-center justify-between">
                  ...
                  <div className="bg-black/20 border border-white/10 rounded-lg px-3 py-1.5 flex items-center">
                    <span className="text-slate-500 text-sm">Search not implemented</span>
                  </div>
                </div>
                <div className="overflow-x-auto max-h-[600px] overflow-y-auto custom-scrollbar">
                  <table className="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {data.global_stats?.all_applications?.slice(0, 50).map((app, idx) => (
                        <tr key={idx} className="hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-6 py-4 text-slate-400 font-mono text-xs">{app.applywizz_id || '--'}</td>
                          <td className="px-6 py-4">
                            <p className="text-white">{app.job_title}</p>
                            <p className="text-slate-500 text-xs mt-0.5">{app.company}</p>
                          </td>
                          <td className="px-6 py-4">
                            <span className="px-2.5 py-1 bg-white/10 text-slate-300 text-[10px] uppercase font-bold rounded-full">
                              {app.status}
                            </span>
                          </td>
                          ...
                          <td className="px-6 py-4 text-right">
                            <a href={app.job_url} target="_blank" rel="noreferrer" className="text-indigo-400 hover:text-indigo-300 text-xs font-medium px-3 py-1.5 bg-indigo-500/10 rounded-lg transition-colors">
                              View ↗
                            </a>
                          </td>
                        </tr>
